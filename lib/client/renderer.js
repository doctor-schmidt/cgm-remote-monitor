...
function init (client, d3) {
  ...
  // These helpers support both legacy raw values and records already
  // serialized by the inbound sanitizer. HTML sinks escape exactly once;
  // text-only sinks decode exactly once before assigning textContent.
  var e = html.textAsHtml;
  var t = html.toTextContent;
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    function focusCircleTooltip (d) {
      ...
      client.tooltip.html('<strong>' + translate('BG') + ':</strong> ' + e(client.sbx.scaleEntry(d)) +
          (d.type === 'mbg' ? '<br/><strong>' + translate('Device') + ': </strong>' + e(d.device) : '') +
          (d.type === 'forecast' && d.forecastType ? '<br/><strong>' + translate('Forecast Type') + ': </strong>' + e(d.forecastType) : '') +
          (rawbgInfo.value ? '<br/><strong>' + translate('Raw BG') + ':</strong> ' + e(rawbgInfo.value) : '') +
          (rawbgInfo.noise ? '<br/><strong>' + translate('Noise') + ':</strong> ' + e(rawbgInfo.noise) : '') +
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    function treatmentTooltip (d) {
      ...
      return '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '<br/>' +
        (d.eventType ? '<strong>' + translate('Treatment type') + ':</strong> ' + e(translate(client.careportal.resolveEventName(d.eventType))) + '<br/>' : '') +
        (d.reason ? '<strong>' + translate('Reason') + ':</strong> ' + e(translate(d.reason)) + '<br/>' : '') +
        (d.glucose ? '<strong>' + translate('BG') + ':</strong> ' + e(d.glucose) + (d.glucoseType ? ' (' + e(translate(d.glucoseType)) + ')' : '') + '<br/>' : '') +
        (d.enteredBy ? '<strong>' + translate('Entered By') + ':</strong> ' + e(d.enteredBy) + '<br/>' : '') +
        (d.targetTop ? '<strong>' + translate('Target Top') + ':</strong> ' + e(targetTop) + '<br/>' : '') +
        (d.targetBottom ? '<strong>' + translate('Target Bottom') + ':</strong> ' + e(targetBottom) + '<br/>' : '') +
        (durationText ? '<strong>' + translate('Duration') + ':</strong> ' + e(durationText) + '<br/>' : '') +
        (d.insulinNeedsScaleFactor ? '<strong>' + translate('Insulin Scale Factor') + ':</strong> ' + d.insulinNeedsScaleFactor * 100 + '%<br/>' : '') +
        (correctionRangeText ? '<strong>' + translate('Correction Range') + ':</strong> ' + e(correctionRangeText) + '<br/>' : '') +
        (d.transmitterId ? '<strong>' + translate('Transmitter ID') + ':</strong> ' + e(d.transmitterId) + '<br/>' : '') +
        (d.sensorCode ? '<strong>' + translate('Sensor Code') + ':</strong> ' + e(d.sensorCode) + '<br/>' : '') +
        (d.notes ? '<strong>' + translate('Notes') + ':</strong> ' + e(d.notes) : '');
    }

    function announcementTooltip (d) {
      return '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '<br/>' +
        (d.eventType ? '<strong>' + translate('Announcement') + '</strong><br/>' : '') +
        (d.notes && d.notes.length > 1 ? '<strong>' + translate('Message') + ':</strong> ' + e(d.notes) + '<br/>' : '') +
        (d.enteredBy ? '<strong>' + translate('Entered By') + ':</strong> ' + e(d.enteredBy) + '<br/>' : '');
    }
    ...
    var treatCircles = chart().focus.selectAll('.treatment-dot').data(client.ddata.treatments.filter(function(treatment) {
      ...
      var notTempOrProfile = !['Temp Basal', 'Profile Switch', 'Combo Bolus', 'Temporary Target'].includes(treatment.eventType);

      var notes = treatment.notes || '';
      var enteredBy = treatment.enteredBy || '';

      var notOpenAPSSpam = enteredBy.indexOf('openaps://') === -1 || openAPSSpam.findIndex(function startsWith (spam) {
    ...
    var durationTreatments = client.ddata.treatments.filter(function(treatment) {
      return !treatment.carbs && !treatment.insulin && (treatment.duration || treatment.durationType !== undefined) &&
        !['Temp Basal', 'Profile Switch', 'Combo Bolus', 'Temporary Target'].includes(treatment.eventType);
    });
    ...
    function treatmentText (d) {
      ...
      return t(d.notes || d.reason || d.eventType);
    }
  ...
  function prepareArc (treatment, radius, bolusSettings) {
    if (treatment.enteredBy === "Glucose" || treatment.enteredBy === "GlucoseDirect") {
      var arc_data = [
      ...
      var arc_data = [
        ...
        // PURPLE insulin half-circle on bottom
        { 'element': '', 'color': '#7d5ab6', 'start': 1.5708, 'end': 4.7124, 'inner': 0, 'outer': radius.R1 },
  ...
  function appendTreatments (treatment, arc) {
    ...
    function boluscalcTooltip (treatment) {
      ...
      html += (treatment.boluscalc.profile ? '<strong>' + translate('Profile used') + ':</strong> ' + e(treatment.boluscalc.profile) + '<br/>' : '');
      if (treatment.boluscalc.foods && treatment.boluscalc.foods.length) {
        ...
        for (var fi = 0; fi < treatment.boluscalc.foods.length; fi++) {
          ...
          html += '<tr>';
          html += '<td>' + e(f.name) + '</td>';
          html += '<td>' + (f.portion * f.portions).toFixed(1) + ' ' + e(f.unit) + '</td>';
    ...
    function treatmentTooltip () {
      ...
      client.tooltip.style('display', 'block');
      client.tooltip.html('<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(treatment)) + '<br/>' + '<strong>' + translate('Treatment type') + ':</strong> ' + e(translate(client.careportal.resolveEventName(treatment.eventType))) + '<br/>' +
          (treatment.carbs ? '<strong>' + translate('Carbs') + ':</strong> ' + e(treatment.carbs) + '<br/>' : '') +
          (treatment.protein ? '<strong>' + translate('Protein') + ':</strong> ' + e(treatment.protein) + '<br/>' : '') +
          (treatment.fat ? '<strong>' + translate('Fat') + ':</strong> ' + e(treatment.fat) + '<br/>' : '') +
          ...
          (treatment.enteredinsulin ? '<strong>' + translate('Combo Bolus') + ':</strong> ' + e(treatment.enteredinsulin) + 'U, ' + e(treatment.splitNow) + '% : ' + e(treatment.splitExt) + '%, ' + translate('Duration') + ': ' + e(treatment.duration) + '<br/>' : '') +
          (treatment.glucose ? '<strong>' + translate('BG') + ':</strong> ' + e(glucose) + (treatment.glucoseType ? ' (' + e(translate(treatment.glucoseType)) + ')' : '') + '<br/>' : '') +
          (treatment.enteredBy ? '<strong>' + translate('Entered By') + ':</strong> ' + e(treatment.enteredBy) + '<br/>' : '') +
          (treatment.notes ? '<strong>' + translate('Notes') + ':</strong> ' + e(treatment.notes) : '') +
      ...
      .on('end', function() {
        ...
        switch (operation) {
          ...
          case 'Move insulin':
            ...
            break;
          case 'Move carbs':            if (window.confirm(translate('Change carbs time to %1 ?', { params: [newTime.toLocaleTimeString()] }))) {
              client.socket.emit(
                'dbUpdateUnset', {
                  collection: 'treatments'
                  , _id: treatment._id
  ...
  renderer.drawTreatments = function drawTreatments (client) {
    ...
    client.ddata.treatments.forEach(function eachTreatment (d) {
  ...
  renderer.addBasals = function addBasals (client) {
    ...
    profile.getBasalRenderTimes(from, to, times.min().msecs).forEach(function renderBasal (date) {
    ...
    g.append('path')
      ...
      .attr('stroke-width', 1)
      .attr('d', area);
    client.ddata.tempbasalTreatments.forEach(function eachTemp (t) {
  ...
  renderer.addTreatmentProfiles = function addTreatmentProfiles (client) {
    ...
    function profileTooltip (d) {
      return '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '<br/>' +
        (d.eventType ? '<strong>' + translate('Treatment type') + ':</strong> ' + e(translate(client.careportal.resolveEventName(d.eventType))) + '<br/>' : '') +
        (d.endprofile ? '<strong>' + translate('End of profile') + ':</strong> ' + e(d.endprofile) + '<br/>' : '') +
        (d.profile ? '<strong>' + translate('Profile') + ':</strong> ' + e(d.profile) + '<br/>' : '') +
        (d.duration ? '<strong>' + translate('Duration') + ':</strong> ' + e(d.duration) + translate('mins') + '<br/>' : '') +
        (d.enteredBy ? '<strong>' + translate('Entered By') + ':</strong> ' + e(d.enteredBy) + '<br/>' : '') +
        (d.notes ? '<strong>' + translate('Notes') + ':</strong> ' + e(d.notes) : '');
    }
    ...
    client.ddata.profileTreatments.forEach(function eachTreatment (d) {
    ...
    var generateText = function(t) {
      ...
      var ret;
      if (t.cutting) {
        ret = sign + '    ' + html.toTextContent(t.cutting) + '    ' + '►►►' + '    ' + html.toTextContent(t.profile) + '    ' + sign;
      } else {
        ret = sign + '    ' + html.toTextContent(t.profile) + '    ' + sign;
      }
      return ret;
    };
